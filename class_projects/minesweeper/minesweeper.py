    ...
    def add_knowledge(self, cell, count):
        ...
        for x in range(cell[0] - 1, cell[0] + 2):
            for y in range(cell[1] - 1, cell[1] + 2):
                ...
                # if we have the current cell, skip
                if (x, y) == cell:
                    continue

                    # add any cells that are unknown
                if 0 <= x < self.height and 0 <= y < self.width:
                    if (x, y) not in self.moves_made and (x, y) not in self.mines:
                        cells.add((x, y))

                    # exclude cells that are known to be mines and drop count by 1
        ...
        for sentence in self.knowledge:
            for block in sentence.cells.copy():
                if block in self.mines:
                    sentence.mark_mine(block)
                if block in self.safes:
                    sentence.mark_safe(block)
            ...
            for sentence1 in self.knowledge:
                ...
                # Run checks on the new knowledge
            for sentence in new_knowledge:
                for block in sentence.cells.copy():
                    if block in self.mines:
                        sentence.cells.remove(block)
                        sentence.count -= 1
                    if block in self.safes:
                        sentence.cells.remove(block)
                    if sentence.count == len(sentence.cells) and sentence.count != 0:
...
# if we have a novel sentence, add it to new knowledge
    ...
    def make_random_move(self):
        ...
        try:
            rando = random.randint(0, len(possible_moves) - 1)
            return possible_moves[rando]
        except ValueError:
            return None
