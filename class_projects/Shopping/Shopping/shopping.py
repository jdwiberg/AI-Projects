...
def load_data(filename):
    ...
    """
    with open(f"{filename}", 'r') as csv_reader:
        ...
        for row in data:
            ev = []
            ev.extend([int(row[0]), float(row[1]), int(row[2]), float(row[3]),
                      int(row[4]), float(row[5]), float(row[6]), float(row[7]),
                      float(row[8]), float(row[9])])
            ev.append(months.index(f"{row[10]}"))
...
def evaluate(labels, predictions):
    ...
    sens = true_pos / total_pos if total_pos > 0 else 0
    spec = true_neg / total_neg if total_neg > 0 else 0
